 ...
 * The constructor should return either a list of functions (none anonymous),
...
$.plugin = function (constructor) {
	...
	if( Object.IsArray(plugin)
		&& (nn = plugin.length) > 0)
		for(; i < nn; i++)
			load(plugin[i].name, plugin[i])
	else
		for( i in (keys = Object.Keys(plugin, true)) )
			load(keys[i], plugin[keys[i]])
...
$.plugin(function Core() {
	...
	return [
		...
		function delay(n, f) {
			...
			return this
			/* Example:
				> $("pre").delay(50, function sometimeLater() {
...
$.plugin(function Transform() {
	...
	return [
		...
		function transform(end_css, speed, easing, callback) {
		...
		function fadeOut(speed, callback, _x, _y) {
